...
class MultiScan(nn.Module):
    """Multi-directional scanner for processing tensors in various directions.

    This module supports scanning tensors in horizontal, vertical, and cluster-based directions,
    with optional flipping for each direction. It's designed to capture multi-scale spatial
    dependencies in input tensors.
    """
...
class BiAttn(nn.Module):
    """Bimodal attention mechanism for channel selection.

    This module performs channel-wise attention by combining global information
    with local features, allowing the model to dynamically adjust channel importance.
    """
...
class MultiMamba(nn.Module):
    """Multi-directional Mamba model with selective scanning.

    This implementation extends the Mamba architecture to process input in multiple
    directions simultaneously, capturing diverse spatial dependencies through
    the MultiScan module. It incorporates depthwise convolution and selective scanning
    operations for efficient sequence modeling.
    """
    def __init__(
            self,
            d_model,
...
class SelectiveScanOflex(torch.autograd.Function):
    """Optimized selective scan operation with autograd support.

    This custom autograd function implements the selective scan operation
    with optimizations for performance, including support for CUDA acceleration
    and automatic mixed precision training.
    """
    @staticmethod
...
class MultiVMamba(nn.Module):
    """Variant of MultiMamba with optimized selective scan implementation.

    This version uses the SelectiveScanOflex function for potentially faster
    computation while maintaining the multi-directional scanning capability
    of the original MultiMamba model.
    """
    def __init__(
            self,
            d_model,
...
def pairwise_cos_sim(x1: torch.Tensor, x2: torch.Tensor):
    ...
    Args:
        x1: First input tensor of shape [B,...,M,D]
        x2: Second input tensor of shape [B,...,N,D]

    Returns:
        Similarity matrix of shape [B,...,M,N]
    """
    """
...
class LocalCluster(nn.Module):
    """Local clustering module for capturing spatial relationships.

    This module performs local clustering on input features to group similar
    features together, enhancing the model's ability to capture local spatial
