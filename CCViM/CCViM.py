...
from models.CCViM.mamba.multi_mamba import MultiScan
...
def flops_selective_scan_fn(B=1, L=256, D=768, N=16, with_D=True, with_Z=False, with_Group=True, with_complex=False):
    """Calculate FLOPs for selective scan operation.
    ...
    Args:
        ...
        D: Model dimension
        N: State dimension
        with_D: Include contribution from D parameter
        with_Z: Include contribution from Z parameter
        with_Group: Include group convolution contribution
        with_complex: Use complex numbers (not supported)
...
class SelectiveScan(torch.autograd.Function):
    """Custom autograd function for selective scan operation.

    This implementation supports both 'mamba_ssm' and 'sscore' modes, with
    optimizations for CUDA acceleration and automatic mixed precision training.
    """
...
class MultiScanVSSM(MultiScan):
    """Multi-directional scanner for Vision Mamba (VSSM).

    Extended version of MultiScan optimized for visual data processing,
    supporting various scanning directions and integrating attention mechanisms
...
class BiAttn(nn.Module):
    """Bimodal attention mechanism for channel and spatial selection.

    This attention module combines global and local information to dynamically
    adjust feature importance both channel-wise and spatially.
    """
...
def multi_selective_scan(
    ...
    """Perform multi-directional selective scan operation.

    This function applies selective scan across multiple directions using
    the provided parameters and returns the processed features.

    Args:
        x: Input tensor
        x_proj_weight: Projection weights
        x_proj_bias: Projection bias
        dt_projs_weight: Delta projection weights
        dt_projs_bias: Delta projection bias
        A_logs: Logarithm of A parameters
        Ds: D parameters
        out_norm: Output normalization module
        nrows: Number of rows for parallel computation
        delta_softplus: Apply softplus to delta
        to_dtype: Convert output to input dtype
        multi_scan: MultiScan module instance
        win_size: Window size for scanning

    Returns:
        Processed tensor after multi-directional selective scan
    """:
    B, D, H, W = x.shape
    D, N = A_logs.shape
...
class PatchMerging2D(nn.Module):
    """Patch merging module for downsampling feature maps.

    This module merges non-overlapping 2x2 patches into a single feature vector,
    reducing spatial dimensions while increasing channel dimension.
    """
...
class SS2D(nn.Module):
    """2D Selective Scan module for visual data processing.

    Core component of the CCViM architecture, combining depthwise convolution
    with multi-directional selective scan operations for efficient and effective
...
class Permute(nn.Module):
    """Simple permutation layer for tensor dimension reordering.

    Utility module to permute tensor dimensions according to the provided order.
    """
...
class Mlp(nn.Module):
    ...
    Standard MLP block with two linear layers and an activation function,
    supporting both channel-first and channel-last tensor formats.
    """
...
class VSSBlock(nn.Module):
    """Vision Selective Scan (VSS) block.

    Basic building block of the CCViM architecture, consisting of normalization,
    SS2D module, and residual connection with drop path regularization.
    """
    def __init__(
            self,
            hidden_dim: int = 0,
...
class PatchExpand2D(nn.Module):
    """Patch expansion module for upsampling feature maps.

    This module expands features to increase spatial dimensions while reducing
    channel dimension, typically used in decoder components.
    """
...
class Backbone_LocalVSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer=nn.LayerNorm, **kwargs):
        ...
        # 删除classifier属性的代码已移除，因为VSSM类中未定义此属性
...
@register_model
def CCViM(in_chans=3,num_classes=1,dims=[96, 192, 384, 768],                     depths=[2, 2, 2, 2],depths_decoder=[2,2,2,1],dims_decoder=[768, 384, 192, 96],                     d_state=16, drop_path_rate=0.2, **kwargs):
    directions = [
        ['h', 'v', 'v_flip', 'c5'],
        ['h', 'h_flip', 'v', 'c5'],
        ['h_flip', 'v', 'v_flip', 'c5'],
